...
def generate_U_tensor_SK(n_orb, U, JH):
    ...
    num_elem = 0
    for iorb1, iorb2, iorb3, iorb4 in product(range(n_orb), repeat=4):
        coeff = 0.0
        ...
        for isp, isp2 in product(range(nsp), repeat=2):
...
line = 0
for iorb1, iorb2, iorb3, iorb4 in product(range(n_site), repeat=4):
    for isp, isp2 in product(range(nsp), repeat=2):
