  ...
  void validate() const{
    ...
    for(int i=0;i<orbitals_;++i){
      for(int j=0;j<orbitals_;++j){
        if(maxAbsCoeff(anti_commutator(c_ops[i], c_ops[j]))>1.e-5) throw std::runtime_error("fermionic operators do not behave as expected: c ops");
        if(maxAbsCoeff(anti_commutator(cdag_ops[i], cdag_ops[j]))>1.e-5) throw std::runtime_error("fermionic operators do not behave as expected: cdag ops");
        if(maxAbsCoeff(sparse_matrix_t(anti_commutator(cdag_ops[i], c_ops[j])-((i==j) ? identity : zero)))>1.e-5){
