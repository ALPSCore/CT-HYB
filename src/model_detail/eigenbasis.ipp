...
template<typename SCALAR>
ImpurityModelEigenBasis<SCALAR>::ImpurityModelEigenBasis(const alps::params& par, const std::vector<boost::tuple<int,int,SCALAR> >& nonzero_t_vals_list,
...
template<typename M>
void remove_high_energy_states(std::vector<std::vector<double> >& evals_sectors, std::vector<M>& evecs_sectors, double cutoff_energy) {
...
min_max_energy_sectors(const std::vector<std::vector<double> >& evals_sectors, std::vector<double>& min_eigenval_sector) {
  const int num_sectors = evals_sectors.size();
...
void ImpurityModelEigenBasis<SCALAR>::build_basis(const alps::params& par) {
  ...
  remove_high_energy_states(eigenvals_sector, evecs_sector, eigenvalue_min+par["CUTOFF_ENERGY_INNER"].template as<double>());
  boost::tie(eigenvalue_max,eigenvalue_min) = min_max_energy_sectors(eigenvals_sector, min_eigenval_sector);
  if (Base::verbose_) {
    print_sectors(eigenvals_sector);
    std::cout << " Max eigen energy = " << eigenvalue_max << std::endl;
  ...
  const double BETA = par["BETA"].template as<double>();
  ...
  for (int flavor=0; flavor<flavors; ++flavor) {
    ...
    for (int src_sector=0; src_sector<num_sectors; ++src_sector) {
      ...
      if (!is_sector_active(dst_sector) || !is_sector_active(src_sector)) {
        ...
        dense_matrix_t tmp_mat = evecs_sector[dst_sector].adjoint()*Base::creation_operators_hyb(flavor,src_sector)*evecs_sector[src_sector];
      ...
      if (!is_sector_active(dst_sector) || !is_sector_active(src_sector)) {
        ...
        dense_matrix_t tmp_mat = evecs_sector[dst_sector].adjoint()*Base::annihilation_operators_hyb(flavor,src_sector)*evecs_sector[src_sector];
...
void ImpurityModelEigenBasis<SCALAR>::build_outer_braket(const alps::params &par) {
  namespace bll = boost::lambda;
  const double cutoff_outer = par["CUTOFF_ENERGY_OUTER"].template as<double>()+*std::min_element(min_eigenval_sector.begin(), min_eigenval_sector.end());
  int active_sector = 0;
  bra_list.resize(0);
  ket_list.resize(0);
  for(int sector=0; sector<Base::num_sectors(); ++sector) {
    ...
    const int dim_outer = std::count_if(eigenvals_sector[sector].begin(), eigenvals_sector[sector].end(), bll::_1<=cutoff_outer);
    if (dim_outer==0) {
      continue;
    }
...
void ImpurityModelEigenBasis<SCALAR>::check_evecs(const std::vector<dense_matrix_t> ham_sector,
  ...
  for (int sector=0; sector<num_sectors; ++sector) {
    for (int ie=0; ie<eigenvals_sector[sector].size(); ++ie) {
      const double res_norm = (ham_sector[sector]*evecs_sector[sector].col(ie)-eigenvals_sector[sector][ie]*evecs_sector[sector].col(ie)).squaredNorm();
...
void ImpurityModelEigenBasis<SCALAR>::sector_propagate_ket(BRAKET_T& ket, double t) const {
  ...
  for (int i=0; i<dim; ++i) {
    assert(eigenvals_sector[sector].size()>i);
    exp_v[i] = std::exp(-t*eigenvals_sector[sector][i]);
  }
