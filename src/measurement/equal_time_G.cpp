...
template class EqualTimeG2Meas<double,SW_REAL_MATRIX>;
template class EqualTimeG2Meas<std::complex<double>,SW_COMPLEX_MATRIX>;
...
void compute_equal_time_G2(
    ...
    std::map<std::string,boost::any> &ar,
    bool verbose) {
  std::vector<double> data_Re = results["Equal_time_G2_Re"].template mean<std::vector<double> >();
  std::vector<double> data_Im = results["Equal_time_G2_Im"].template mean<std::vector<double> >();
  double coeff = worm_space_rel_vol/(sign * beta);

  check_true(data_Re.size() == n_flavors * n_flavors * n_flavors * n_flavors);
  boost::multi_array<std::complex<double>, 4> data(boost::extents[n_flavors][n_flavors][n_flavors][n_flavors]);
  std::transform(data_Re.begin(), data_Re.end(), data_Im.begin(), data.origin(), to_complex<double>());
  std::transform(data.origin(), data.origin() + data.num_elements(), data.origin(),
                 std::bind1st(std::multiplies<std::complex<double> >(), coeff));
