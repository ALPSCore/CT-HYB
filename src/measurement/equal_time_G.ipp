...
template <typename SCALAR, typename SW_TYPE>
void 
EqualTimeG2Meas<SCALAR,SW_TYPE>::measure(
      const MonteCarloConfiguration<SCALAR> &mc_config,
      const SW_TYPE &sliding_window,
      alps::accumulators::accumulator_set &measurements)
{
  boost::multi_array<std::complex<double>,4> obs_(boost::extents[nflavors_][nflavors_][nflavors_][nflavors_]);
  std::fill(obs_.origin(), obs_.origin()+obs_.num_elements(), 0.0);
  obs_[mc_config.p_worm->get_flavor(0)]
      [mc_config.p_worm->get_flavor(1)]
      [mc_config.p_worm->get_flavor(2)]
      [mc_config.p_worm->get_flavor(3)] = mc_config.sign;

  measure_simple_vector_observable<std::complex<double>>(
    measurements, "Equal_time_G2", to_std_vector(obs_)
  );
};