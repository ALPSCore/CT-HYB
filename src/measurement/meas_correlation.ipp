...
MeasCorrelation<SW, OBS>::MeasCorrelation(const std::vector<std::pair<OBS, OBS> > &correlators, int num_tau_points)
  ...
  for (typename std::vector<std::pair<OBS, OBS> >::const_iterator it = correlators.begin(); it != correlators.end();
       ++it) {
...
template<typename SW, typename OBS>
void
MeasCorrelation<SW, OBS>::perform_meas(SW &sw,
                                       const operator_container_t &operators,
                                       boost::multi_array<EXTENDED_COMPLEX, 2> &result) const {
