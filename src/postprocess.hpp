                              ...
                              const typename alps::parameters_type<SOLVER_TYPE>::type& parms, alps::hdf5::archive ar)
{
  ...
  ITIME_GF itime_gf(alps::gf::itime_mesh(beta, n_tau+1), alps::gf::index_mesh(n_flavors), alps::gf::index_mesh(n_flavors));
    ...
    for (int flavor=0; flavor<n_flavors; ++flavor) {
      for (int flavor2=0; flavor2<n_flavors; ++flavor2) {
        for (int il=0; il<n_legendre; ++il) {
          itime_gf(g::itime_index(itau), g::index(flavor), g::index(flavor2) ) += Pvals[il]*Gl[flavor][flavor2][il]*sqrt_array[il]*temperature/sign;
        }
      }
    }
  }
  ...
  GOMEGA gomega(alps::gf::matsubara_positive_mesh(beta, n_tau), alps::gf::index_mesh(n_flavors), alps::gf::index_mesh(n_flavors));
  for (int flavor=0; flavor<n_flavors; ++flavor) {
    for (int flavor2 = 0; flavor2 < n_flavors; ++flavor2) {
      for (int il=0; il<n_legendre; ++il) {
