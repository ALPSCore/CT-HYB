...
def _eval_g(F1, F2, B, beta, wsample, eigen_enes, eigen_vecs):
    ...
    There is a typo: The sign in the second term must be "-" due to the excanhge of feromionic operators F_1 and F_2.
    """
    ...
    term1 = _eval_first_term_g(F1_, F2_, B_, beta, (v,vp), eigen_enes)
    term2 = _eval_first_term_g(F2_, F1_, B_, beta, (vp,v), eigen_enes)

    return np.asarray(term1 - term2)


def _eval_first_term_g(F1, F2, B, beta, wsample, eigen_enes):
    """
    Implements the first term of Eq. (C2)
    We take care of the singular terms.
    """
    v = check_fermionic(wsample[0])
    vp = check_fermionic(wsample[1])
    ...
    frac13 = (expE[None,:,None] + expE[None,None,:])/(iv[:,None,None] + Ediff[None,:,:])
    term1 = \
